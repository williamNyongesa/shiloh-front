...
const EventsPage = () => {
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        const response = await axios.get('http://localhost:5000/events');  // Replace with actual API endpoint
  ...
  const handleCreateEvent = async () => {
    ...
    try {
      const response = await axios.post('http://localhost:5000/events/submit-event', newEvent);
